...
<select class="form-control col-md-12" name="topic" required>
    ...
    <?php foreach ($BeginnerChildren as $bChild){ ?>
        <option value="<?php echo $beginnerPage->post_title . "-" . $bChild->post_title ?>" <?php if($edit_level==$beginnerPage->post_title && $edit_topic == $bChild->post_title){ echo "selected" ;} ?>><?php echo $beginnerPage->post_title . " - " . $bChild->post_title ?></option>
    <?php } foreach ($IntermediateChildren as $iChild){ ?>
        <option value="<?php echo $intermediatePage->post_title . "-" . $iChild->post_title ?>" <?php if($edit_level==$intermediatePage->post_title && $edit_topic == $iChild->post_title){ echo "selected" ;} ?>><?php echo $intermediatePage->post_title . " - " . $iChild->post_title ?></option>
    <?php } foreach ($AdvanceChildren as $aChild){ ?>
        <option value="<?php echo $advancePage->post_title . "-" . $aChild->post_title ?>" <?php if($edit_level==$advancePage->post_title && $edit_topic == $aChild->post_title){ echo "selected" ;} ?>><?php echo $advancePage->post_title . " - " . $aChild->post_title ?></option>
    <?php } ?>
